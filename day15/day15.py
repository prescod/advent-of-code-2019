...
class Map:
    ...
    def moved_foward(self, command):
        ...
        for neighbour in self.adjacents(pos):
            if self.locations.get(neighbour) == SPACE:
                print("Adding Edge: ", (self.droid_pos, neighbour))
    ...
    def next_step(self):
        ...
        path_to_adventure = self.find_shortest_path_to_unexplored(self.droid_pos)
        if path_to_adventure: 
            self.steps = list(path_to_adventure)[1:]
    ...
    def find_shortest_path_to_unexplored(self, pos):
        for visiting_node in networkx.bfs_tree(self.connections, pos):
            if visiting_node in self.unexplored_spaces:
                assert pos != visiting_node
                return shortest_path(self.connections, pos, visiting_node)
        self.done()
